...
基于 Spring Cloud Alibaba 的微服务实战项目，演示了完整的电商核心功能：用户认证、商品管理、购物车、订单、支付、秒杀、商品评论、**AI 智能体**等，并集成了服务注册与发现、配置中心、API 网关、熔断降级、分布式锁、消息队列、**大模型 Function Calling** 等主流技术。
...
| JWT | 0.11.5 | 身份认证 |
| Lombok | 1.18.30 | 简化代码 |
| LangChain4j | 0.35.0 | AI 应用开发框架（声明式 AI 服务、Function Calling） |
...
├── good-service/ // 商品服务（商品查询、库存操作、商品评论）
├── order-service/ // 订单服务（购物车、订单、支付、秒杀）
├── ai-service/ // AI 智能体服务（大模型对话、Function Calling）
...
4. 初始化数据库
数据库表结构请参考各服务模块的 mapper 文件或手动执行 SQL。
...
ai-service（AI 智能体服务，端口 8087）

gateway（网关，端口 8080）

6. 测试接口（推荐 Postman 或 IDEA HTTP Client）
详见下方接口示例。
...
9. AI 智能体模块（新增）
集成 LangChain4j 框架，通过 @AiService 声明式定义 AI 服务

实现 Function Calling，AI 可自动调用订单查询、评论查询等微服务接口

通过 Feign 获取真实业务数据，AI 生成自然语言总结

结合 Sentinel 进行限流，防止大模型 API 调用费用失控

支持多轮对话上下文记忆（按 sessionId 隔离）
...
http
POST http://localhost:8080/ai/chatWithTools
Content-Type: application/json

{
    "sessionId": "test1",
    "message": "帮我查一下订单 837357036881711104 的详细信息"
}
🧪 压测与验证
限流测试（Sentinel）：使用 JMeter 或 Postman Runner 连续快速请求 /good/reduceStock，超出 QPS 阈值后返回 429

秒杀并发测试：模拟 50 个线程同时秒杀，观察 Redis 库存原子递减、不超卖，同一用户只能秒杀一次，秒杀成功订单异步创建

AI 限流测试：高频调用 /ai/chat 或 /ai/chatWithTools，触发 Sentinel 限流规则，验证降级提示
...
AI 模块的 API-Key 存放在 Nacos 配置中心（ai-service-dev.yaml），避免硬编码

使用百炼 API 需确保网络连通，测试前请先在 Nacos 中正确配置 api-key
